...
<div class="list-group">
	<c:forEach var="node" items="${docs.resultList}">
		<a href="webquest/fqa/Pub${node.QUESTIONID}.html" class="list-group-item"
			target="${config_sys_link_newwindow_target}">
			<div class="row">
				<div class="col-md-7">
					<h5 class="list-group-item-heading">${node.TITLE}&nbsp;${node.QUESTIONSTATE == "完成" ? "<span title='已完成' class='label label-success' style='font-size:10px;'><span class='glyphicon glyphicon-ok'></span></span>" : ""}</h5>
				</div>
				<div class="col-md-5">
					<p class="list-group-item-text" >
					${node.CTIME}&nbsp;${node.PSTATE == "已经采纳" ? "<span class='label label-success'>已采纳</span>" : "<span class='label label-primary'>未采纳</span>"}</p>
				</div>
			</div>
		</a>
	</c:forEach>
