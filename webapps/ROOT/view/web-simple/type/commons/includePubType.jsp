...
<script type="text/javascript">
	var types;
	$(function() {
		$.post('webtype/PubLoadRType.html?typeDomainId=${typedomain.id}',{typeid:'${typeid}'}, function(flag) {
	...
	function clickType(obj){ 
		if ($(obj).attr('id')) {
			if ($(obj).hasClass("linklevel1") ){
				window.location = basePath + "webtype/view"+ $(obj).attr('id') + "/Pub1.html?typeDomainId="+$(obj).attr('id');
			}else{
				window.location = basePath + "webtype/view"+ $(obj).attr('id') + "/Pub1.html?typeDomainId=${typedomain.id}";
			}
		}
	}
</script>