...
<div class="alert alert-warning" role="alert">原版本注释：${CTEXT.pcontent}</div>
<!-- 自定义表单展示 -->
...
<div class="row">
	<div class="col-md-12">
		<div style="text-align: center;">
			<div class="row">
				<c:forEach var="node" items="${CFILES}">
					<div class="col-md-4">
						<div class="panel panel-default">
							<div class="panel-body " style="padding-bottom: 4px;">
								<div class="parent">
									...
									<c:if
										test="${fn:toUpperCase(fn:replace(node.exname,'.',''))!='PNG'&&fn:toUpperCase(fn:replace(node.exname,'.',''))!='JPG'&&fn:toUpperCase(fn:replace(node.exname,'.',''))!='JPEG'&&fn:toUpperCase(fn:replace(node.exname,'.',''))!='GIF'}">
										<img class="child" style="max-height: 128px; max-width: 100%;"
											alt="${node.name}"
											src="text/img/fileicon/${fn:toUpperCase(fn:replace(node.exname,'.',''))}.png" />
									</c:if>
								</div>
								<div
									style="white-space: nowrap; color: #666; overflow: hidden; text-overflow: ellipsis;">${node.name}</div>
							</div>
							<div class="panel-footer">
								<div style="overflow: hidden; height: 20px;">
									<span style="color: #666;">${node.len/1000}kb</span>
									&nbsp;&nbsp;&nbsp;&nbsp; <a title="下载文件"
										class="downloadButton_wcp" href="${node.url}"> <span
...
<c:if test="${CTEXT.text!=null&&CTEXT.text!=''}">
	<div class="panel panel-default">
		<div class="panel-body">
			<div class="row">
				<div class="col-md-12" id="textsBoxId">
					...
					<p class="ke-content">${CTEXT.text}</p>
				</div>
			</div>
		</div>
	</div>
