...
<body class="easyui-layout">
		...
		<form id="searchMessagemodelForm">
			<table class="editTable">
				<tr>
					...
					<td><input name="TYPEKEY:like" type="text"></td>
					<td class="title">标题模板:</td>
					<td><input name="TITLEMODEL:like" type="text"></td>
		...
		<table id="dataMessagemodelGrid">
			<thead>
				<tr>
					...
					<th field="BEANTITLES" data-options="sortable:false" width="40">发送方式</th>
	...
	<div id="messageModelToolbar">
		<a class="easyui-linkbutton"
			data-options="iconCls:'icon-limited-edition',plain:true,onClick:initMessageModel">初始化
		</a> <a class="easyui-linkbutton"
			data-options="iconCls:'icon-edit',plain:true,onClick:editDataMessagemodel">修改模板
		</a> <a class="easyui-linkbutton"
			data-options="iconCls:'icon-my-account',plain:true,onClick:setSenders">设置抄送人
		</a> <a class="easyui-linkbutton"
			data-options="iconCls:'icon-reload',plain:true,onClick:reFrashMessagemodelCache">刷新缓存</a>
		<a class="easyui-linkbutton"
			data-options="iconCls:'icon-documents_email',plain:true,onClick:editMessageSender">设置发送方式
		</a> <a href="javascript:void(0)" id="mb" class="easyui-menubutton"
			data-options="menu:'#mm5',iconCls:'icon-edit'">设置状态</a>
		<div id="mm5" style="width: 150px;">
			<div data-options="iconCls:'icon-upcoming-work'"
				onclick="resetModelState(true)">启用</div>
			<div data-options="iconCls:'icon-busy'"
				onclick="resetModelState(false)">禁用</div>
		</div>
	</div>
</body>
<script type="text/javascript">
	...
	//设置发送方式
	function editMessageSender() {
		var selectedArray = $(gridMessagemodel).datagrid('getSelections');
		if (selectedArray.length > 0) {
			var url = 'messagemodel/editBeans.do?ids='
