/**
 * User: Jinqn Date: 14-04-08 Time: 下午16:34 上传图片对话框逻辑代码,包括tab:
 * 远程图片/上传图片/在线图片/搜索图片
 */
...
(function() {
	...
	function RemoteImage(target) {
		this.container = utils.isString(target) ? document
				.getElementById(target) : target;
		this.init();
	}
	RemoteImage.prototype = {
		...
		initEvents : function() {
			...
			domUtils.on($G("lock"), 'change', function() {
				var proportion = parseInt($G("width").value)
						/ parseInt($G("height").value);
				locker.setAttribute('data-proportion', proportion);
			});
			// 初始化约束图片比例为选中--二次開發
			var proportion = parseInt($("#width").val())/ parseInt($("#height").val());
			$("#lock").attr('data-proportion', proportion);
		...
		updateLocker : function() {
			var width = $G('width').value, height = $G('height').value, locker = $G('lock');
			if (width && height && width == parseInt(width)
		...
		setImage : function(img) {
			...
			var wordImgFlag = img.getAttribute("word_img"), src = wordImgFlag ? wordImgFlag
					.replace("&amp;", "&")
					: (img.getAttribute('_src') || img.getAttribute("src", 2)
							.replace("&amp;", "&")), align = editor
					.queryCommandValue("imageFloat");
		...
		setPreview : function() {
			var url = $G('url').value, ow = parseInt($G('width').value, 10) || 0, oh = parseInt(
					$G('height').value, 10) || 0, border = parseInt(
					$G('border').value, 10) || 0, title = $G('title').value, preview = $G('preview'), width, height;

			url = utils.unhtmlForUrl(url);
			title = utils.unhtml(title);
			//二次开发，控制弹出框中的缩略图不变化
			//width = ((!ow || !oh) ? preview.offsetWidth : Math.min(ow,
			//		preview.offsetWidth));
			//width = width + (border * 2) > preview.offsetWidth ? width
			//		: (preview.offsetWidth - (border * 2));
			//height = (!ow || !oh) ? '' : width * oh / ow;
			width='100%';
			height='';
			if (url) {
				preview.innerHTML = '<img src="' + url + '" width="' + width
						+ '" height="' + height + '" border="' + border
						+ 'px solid #000" title="' + title + '" />';
			}
		},
	...
	UploadImage.prototype = {
		...
		initUploader : function() {
			var _this = this, $ = jQuery, // just in case. Make sure it's not
			// an other libaray.
			$wrap = _this.$wrap,
			// 图片容器
			...
			// WebUploader实例
			uploader, actionUrl = editor.getActionUrl(editor
			...
			if (!WebUploader.Uploader.support()) {
				...
				$('#filePickerReady').after(
						$('<div>').html(lang.errorLoadConfig)).hide();
				return;
			}
			...
			function updateStatus() {
				...
				if (state === 'ready') {
					text = lang.updateStatusReady.replace('_', fileCount)
		...
		getInsertList : function() {
			var i, data, list = [], align = getAlign(), prefix = editor
	...
	OnlineImage.prototype = {
		...
		initEvents : function() {
			...
			/* 滚动拉取图片 */
			domUtils.on($G('imageList'), 'scroll',
					function(e) {
						var panel = this;
						if (panel.scrollHeight
								- (panel.offsetHeight + panel.scrollTop) < 10) {
							_this.getImageData();
						}
					});
			/* 选中图片 */
			domUtils.on(this.container, 'click', function(e) {
				var target = e.target || e.srcElement, li = target.parentNode;
		...
		getImageData : function() {
			...
			if (!_this.listEnd && !this.isLoadingData) {
				...
				var url = editor.getActionUrl(editor
						.getOpt('imageManagerActionName')), isJsonp = utils
						...
						.request(
								url,
								{
									...
									'onsuccess' : function(r) {
										try {
											var json = isJsonp ? r : eval('('
													+ r.responseText + ')');
											if (json.state == 'SUCCESS') {
												_this.pushData(json.list);
												_this.listIndex = parseInt(json.start)
														+ parseInt(json.list.length);
		...
		pushData : function(list) {
			var i, item, img, icon, _this = this, urlPrefix = editor
					.getOpt('imageManagerUrlPrefix');
			for (i = 0; i < list.length; i++) {
				if (list[i] && list[i].url) {
					...
					domUtils.on(img, 'load', (function(image) {
						return function() {
							_this.scale(image, image.parentNode.offsetWidth,
		...
		scale : function(img, w, h, type) {
			...
			if (type == 'justify') {
				if (ow >= oh) {
					img.width = w;
					img.height = h * oh / ow;
					img.style.marginLeft = '-' + parseInt((img.width - w) / 2)
							+ 'px';
				} else {
					img.width = w * ow / oh;
					img.height = h;
					img.style.marginTop = '-' + parseInt((img.height - h) / 2)
							+ 'px';
				}
			} else {
				if (ow >= oh) {
		...
		getInsertList : function() {
			...
			for (i = 0; i < lis.length; i++) {
				if (domUtils.hasClass(lis[i], 'selected')) {
					var img = lis[i].firstChild, src = img.getAttribute('_src');
					list.push({
						src : src,
						_src : src,
	...
	SearchImage.prototype = {
		...
		initEvents : function() {
			...
			domUtils
					.on(
							...
							'click',
							function(e) {
								var target = e.target || e.srcElement, li = target.parentNode.parentNode;
		...
		getImageData : function() {
			var _this = this, key = $G('searchTxt').value, type = $G('searchType').value, keepOriginName = editor.options.keepOriginName ? "1"
					: "0", url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word="
					+ _this.encodeToGb2312(key)
					+ type
					+ "&keeporiginname="
					+ keepOriginName + "&" + +new Date;
		...
		setList : function(list) {
			var i, item, p, img, link, _this = this, listUl = $G('searchListUl');
		...
		getInsertList : function() {
			var child, src, align = getAlign(), list = [], items = $G('searchListUl').children;
